...
resource "aws_route_table_association" "public_subnet_association" {
  subnet_id      = aws_subnet.emir_subnet.id
...
resource "aws_instance" "emir_instance" {
  ami                    = data.aws_ami.emir_ami.id
  instance_type          = var.instance_type
  subnet_id              = aws_subnet.emir_subnet.id
  key_name               = var.ec2_key
  vpc_security_group_ids = [aws_security_group.emir_security_group.id]
 
  iam_instance_profile = var.ec2_iam_instance_profile
 
  tags = {
    Name = "emirEC2Instance"
  }
...
output "public_ip" {
  value = aws_instance.emir_instance.public_ip
}
